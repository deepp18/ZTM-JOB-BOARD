...
const userSchema = new mongoose.Schema(
  {
    ...
    // Optional: keep the submitted profile on the user
    profile: {
      name: String,
      img: String,
      email: String,
...
// Student profile submit (marks profileCompleted = true)
app.post('/api/profile', auth, async (req, res) => {
  try {
    const u = await User.findById(req.user.userId);
    if (!u) return res.status(404).json({ message: 'User not found' });

    // Save whatever you receive (you already normalized on the client)
    u.profile = req.body && typeof req.body === 'object' ? req.body : {};
    u.profileCompleted = true;
    await u.save();

    const token = signToken(u); // optional: refresh token
    return res.json({
