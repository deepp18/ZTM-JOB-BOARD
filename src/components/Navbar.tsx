...
const Navbar = ({
  ...
  persons,   // ✅ pass persons array from App.tsx
  onFilterApply, // ✅ new callback to apply filters
}: any) => {
  ...
  // ✅ extract unique cities & jobs from persons
const cities: string[] = persons
  ? (Array.from(new Set(persons.map((p: any) => p.city).filter(Boolean))) as string[])
  : [];

const jobs: string[] = persons
  ? (Array.from(new Set(persons.map((p: any) => p.role).filter(Boolean))) as string[])
  : [];
  ...
  return (
    <div className="header-items flex flex-wrap justify-between">
      ...
      <div className="flex items-center">
        {/* Replace Pointer with span that preloads Map on hover and focus */}
        <span

  onMouseEnter={() => import('../components/Map')}
  onFocus={() => import('../components/Map')}
  onClick={onMapClick}
  role="button"
  tabIndex={0}
